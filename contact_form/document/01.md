...
2. 1で作成したディレクトリの中に、**index.php** を作成します。
3. index.phpに、次のように記述します。
...
### 別画面へのデータの送信
作成した`index.php`から`check.php`へのデータの送信に関して、  
①別ページへの移動と、  
②データの送信に分けて説明します。  
...
`index.php`では`form`タグを使用して、`action="check.php"`を指定しています。  
...
`index.php`の以下の記述は、**check.phpにPOSTでリクエストする**ということになります。
`<form method="POST" action="check.php">`

### データの受け取り
`index.php`でニックネームを入力して送信ボタンを押してください。
...
`$_POST['nickname']`に`index.php`のニックネームに入力した内容が入っています。  
...
連想配列のキーは送信元、今回は`index.php`で決まっています。
...
ただ、$nickname, $email, $contentという変数の中には `index.php` のフォームで入力された値が入っています。  
...
### 不正な画面遷移を防ぐ
`check.php`と`thanks.php`は`index.php`から送信ボタンを押してもらって  
